...
export default async function LeadDetailsPage({
  ...
  const formattedDate = new Date(lead.submittedAt).toLocaleString("en-GB", {
    dateStyle: "medium",
    timeStyle: "short",
  });
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="overflow-hidden rounded-2xl border border-green-100 shadow-sm">
          ...
          <div className="flex items-center justify-between bg-[#0a1f14] px-8 py-6">
            ...
            <span
              className={`rounded-full px-3 py-1 text-xs font-semibold capitalize ${statusStyles[lead.status]}`}
            >
              {lead.status}
            </span>
          </div>
          ...
          <div className="divide-y divide-green-50 px-8 py-2">
            ...
            <DetailRow icon={<Briefcase className="h-4 w-4" />} label="Service Needed" value={lead.service} />
            <DetailRow icon={<Calendar className="h-4 w-4" />} label="Submitted" value={formattedDate} />
          </div>
          ...
          <div className="flex gap-3 border-t border-green-100 px-8 py-5">
            ...
            <button className="rounded-lg border border-green-200 px-4 py-2 text-sm font-semibold text-[#0a1f14] transition-colors hover:bg-green-50">
              Mark as Contacted
            </button>
          </div>
        </div>
  ...
  return (
    <div className="flex items-center gap-3 py-4">
      ...
      <div>
        ...
        <p className="text-sm font-medium text-[#0a1f14]">{value}</p>
      </div>
    </div>
  );
}
